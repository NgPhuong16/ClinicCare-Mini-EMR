...
def test_create_returns_201_with_attached_codes(auth_client: TestClient, codes: None) -> None:
    response = auth_client.post(URL, json=_payload())
...
def test_create_unknown_code_returns_422_naming_the_code(
    auth_client: TestClient, codes: None
) -> None:
    response = auth_client.post(URL, json=_payload(diagnosis_codes=["E11.9", "Z99.9"]))
...
def test_create_unknown_code_writes_nothing(
    auth_client: TestClient, codes: None, db: Session
) -> None:
    auth_client.post(URL, json=_payload(diagnosis_codes=["E11.9", "Z99.9"]))

    assert db.execute(select(func.count()).select_from(Consultation)).scalar_one() == 0


def test_create_empty_diagnosis_codes_returns_422(auth_client: TestClient, codes: None) -> None:
    response = auth_client.post(URL, json=_payload(diagnosis_codes=[]))
...
def test_create_blank_patient_name_returns_422(
    auth_client: TestClient, codes: None, name: str
) -> None:
    response = auth_client.post(URL, json=_payload(patient_name=name))
...
def test_create_strips_patient_name(auth_client: TestClient, codes: None) -> None:
    response = auth_client.post(URL, json=_payload(patient_name="  Tran Bao  "))

    assert response.json()["patient_name"] == "Tran Bao"


def test_create_response_diagnoses_order(auth_client: TestClient, codes: None) -> None:
    ...
    response = auth_client.post(URL, json=_payload(diagnosis_codes=["I10", "E11.9"]))

    assert [d["code"] for d in response.json()["diagnoses"]] == ["E11.9", "I10"]


def test_list_response_diagnoses_order_matches_create(auth_client: TestClient, codes: None) -> None:
    created = auth_client.post(URL, json=_payload(diagnosis_codes=["I10", "E11.9"]))
...
def test_create_returns_created_at_as_explicit_utc(auth_client: TestClient, codes: None) -> None:
    response = auth_client.post(URL, json=_payload())

    created_at = response.json()["created_at"]
    # The column is naive UTC, so without the schema validator this would carry no offset
    # and a auth_client would be free to read it as local time.
    assert created_at.endswith("Z")
    assert datetime.fromisoformat(created_at).utcoffset() == timedelta(0)


def test_list_returns_created_at_as_explicit_utc(auth_client: TestClient, db: Session) -> None:
...
def test_list_returns_newest_first(auth_client: TestClient, db: Session) -> None:
    db.add_all(
        [
...
def test_list_filtered_by_patient_narrows(auth_client: TestClient, db: Session) -> None:
    db.add_all([make_consultation("Nguyen An"), make_consultation("Tran Bao")])
    db.commit()

    response = auth_client.get(URL, params={"patient": "nguyen"})

    assert response.status_code == 200
    assert [row["patient_name"] for row in response.json()] == ["Nguyen An"]


def test_list_filtered_by_code_narrows(auth_client: TestClient, db: Session, codes: None) -> None:
    ...
    response = auth_client.get(URL, params={"code": "E11.9"})
...
def test_list_code_filter_applies_before_the_limit(
    auth_client: TestClient, db: Session, codes: None
) -> None:
    ...
    response = auth_client.get(URL, params={"code": "E11.9", "limit": 3})

    assert [row["patient_name"] for row in response.json()] == ["Diabetic"]


def test_list_unknown_code_returns_empty_list(auth_client: TestClient, db: Session) -> None:
    db.add(make_consultation("Nguyen An"))
    db.commit()

    response = auth_client.get(URL, params={"code": "Z99.9"})

    assert response.status_code == 200
    assert response.json() == []


def test_list_respects_limit_and_offset(auth_client: TestClient, db: Session) -> None:
    db.add_all(
        make_consultation(f"Patient {i}", created_at=datetime(2026, 9, 1 + i)) for i in range(5)
    )
    db.commit()

    response = auth_client.get(URL, params={"limit": 2, "offset": 1})

    assert [row["patient_name"] for row in response.json()] == ["Patient 3", "Patient 2"]


def test_list_limit_above_100_returns_422(auth_client: TestClient) -> None:
    response = auth_client.get(URL, params={"limit": 101})
